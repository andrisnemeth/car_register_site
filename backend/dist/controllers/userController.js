  ...
  try {
    ...
    if (!data) {
      return res.status(400).json({
        error: "Nincs felhasználó ezzel az ID-val"
      });
    }
    res.send(data);
  } catch (error) {
...
async function getAllChangedTypeUsersByType(req, res) {
  try {
    ...
    if (data.length === 0) {
      return res.status(204).json({
        error: "Nincs ilyen típusú felhasználó"
      });
    }
    res.send(data);
  } catch (error) {
...
async function registerUser(req, res) {
  ...
  try {
    ...
    if (existingUser) {
      return res.status(400).json({
        error: "Az e-mail cím már foglalt"
      });
    }
    if (!password) {
      return res.status(400).json({
        error: "A jelszó kötelező"
      });
    }
    ...
    await newUser.save();
    const token = generateToken(newUser.id, email, typeOfUser);
    console.log(token);
...
async function loginUser(req, res) {
  ...
  if (!existingUser) {
    return res.status(400).json({
      error: "Az e-mail cím vagy jelszó hibás"
    });
  }
  ...
  const token = generateToken(id, userEmail, typeOfUser);
