...
async function addNewCarBrand(req, res) {
  ...
  try {
    ...
    const existingBrandName = await CarBrand.findOne({
      where: {
        brandName
      }
    });
    if (existingBrandName) {
      return res.status(400).json({
        error: "Az márka már létezik létezik"
      });
    }
    ...
    return res.status(200).json({
      message: "Sikeresen hozzáadta ezt a márkát!"
    });
  } catch (error) {
    console.log(error);
    res.sendStatus(500);
...
async function addNewCarType(req, res) {
  ...
  try {
    ...
    const existingTypeName = await CarType.findOne({
      where: {
        typeName
      }
    });
    if (existingTypeName) {
      return res.status(400).json({
        error: "Az típus már létezik létezik"
      });
    }
    ...
    await newType.save();
    return res.status(200).json({
      message: "Sikeresen hozzáadta ezt a típust!"
    });
  } catch (error) {
    console.log(error);
    res.sendStatus(500);
...
async function uploadCarPicture(req, res) {
  try {
    ...
    res.status(500).json({
      error: "Failed to upload car picture"
    });
  }
}
module.exports = {
